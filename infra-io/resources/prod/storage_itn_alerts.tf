...
resource "azurerm_monitor_diagnostic_setting" "itn_queue_diagnostic_setting" {
  name                       = "io-p-itn-cgn-st-queue-ds-01"
  target_resource_id         = "${data.azurerm_storage_account.storage_cgn_itn.id}/queueServices/default"
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "pending_cgn_failure_alert_rule_itn" {
  enabled             = true
  name                = "[CGN | iopitncgnst01] Failures on pendingcgn-poison"
  resource_group_name = data.azurerm_resource_group.itn_cgn.name
  location            = local.location

  scopes                  = [data.azurerm_storage_account.storage_cgn_itn.id]
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "pending_eyca_failure_alert_rule_itn" {
  enabled             = true
  name                = "[CGN | iopitncgnst01] Failures on pendingeyca-poison"
  resource_group_name = data.azurerm_resource_group.itn_cgn.name
  location            = local.location
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "activated_cgn_failure_alert_rule_itn" {
  enabled             = true
  name                = "[CGN | iopitncgnst01] Failures on activatedcgn-poison"
  resource_group_name = data.azurerm_resource_group.itn_cgn.name
  location            = local.location
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "activated_eyca_failure_alert_rule_itn" {
  enabled             = true
  name                = "[CGN | iopitncgnst01] Failures on activatedeyca-poison"
  resource_group_name = data.azurerm_resource_group.itn_cgn.name
  location            = local.location
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "pendingdelete_cgn_failure_alert_rule_itn" {
  enabled             = true
  name                = "[CGN | iopitncgnst01] Failures on pendingdeletecgn-poison"
  resource_group_name = data.azurerm_resource_group.itn_cgn.name
  location            = local.location
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "pendingdelete_eyca_failure_alert_rule_itn" {
  enabled             = true
  name                = "[CGN | iopitncgnst01] Failures on pendingdeleteeyca-poison"
  resource_group_name = data.azurerm_resource_group.itn_cgn.name
  location            = local.location
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "expired_cgn_failure_alert_rule_itn" {
  enabled             = true
  name                = "[CGN | iopitncgnst01] Failures on expiredcgn-poison"
  resource_group_name = data.azurerm_resource_group.itn_cgn.name
  location            = local.location
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "expired_eyca_failure_alert_rule_itn" {
  enabled             = true
  name                = "[CGN | iopitncgnst01] Failures on expiredeyca-poison"
  resource_group_name = data.azurerm_resource_group.itn_cgn.name
  location            = local.location
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "messages_failure_alert_rule_itn" {
  enabled             = true
  name                = "[CGN | iopitncgnst01] Failures on messages-poison"
  resource_group_name = data.azurerm_resource_group.itn_cgn.name
  location            = local.location
